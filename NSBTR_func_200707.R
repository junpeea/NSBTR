...
Update1_beta_phi = function(Parset_,data00,beta.mu0=0,beta.sig20=100,a=1,b=1,Qo,Qc){
...
gibbs_sampler <- function(X,Y,B0,sigma0,sigma2,theta0,D0,reps,out,out1,p){
  ...
  for(i in 1:reps){
    ...
    for(m in (p+1):horizon){
      ...
      yhat[m] = c(X_mat,as.numeric(data0[(n-2+m),2])) %*% B + rnorm(1) * cfactor
